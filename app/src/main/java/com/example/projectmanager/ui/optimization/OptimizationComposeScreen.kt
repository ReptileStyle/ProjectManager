...
fun OptimizationComposeScreen(
    ...
    if (viewModel.state.isMonteCarlo) {
        MonteCarloScreen(state = viewModel.state, onEvent = viewModel::onEvent)
    } else {
        if (viewModel.state.isPlotVisible) {
            ...
            ConstraintLayout(modifier = Modifier.fillMaxSize()) {
                ...
                if (viewModel.state.plotModel != null) {
                    Row(modifier = Modifier.constrainAs(showPlotButton) {
                        start.linkTo(parent.start)
                        end.linkTo(parent.end)
                        ...
                        Button(
                            onClick = { viewModel.onEvent(OptimizationEvent.OnChooseMonteCarloMode) },
                        ) {
                            Text(text = "Метод Монте-Карло")
                        }
                    }
